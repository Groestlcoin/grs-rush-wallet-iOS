    ...
    "getHistory": function ()
    {
        ...
        }).done(function (msg)
        {
            ...
            for ( i=0;i<msg.txs.length;i++ )
            {
                ...
                var $tr = $('<tr><td>' + txTime + '</td><td class="hidden-sm hidden-xs"><a href="https://chainz.cryptoid.info/grs/tx.dws?' + msg.txs[i].hash + '.htm" target="_blank" >' + msg.txs[i].hash.substring(0,30) + '...</a></td><td class="hidden-sm hidden-xs">' +  formatMoney( msg.txs[i].confirmations || 0 ) + '</td><td>' + btcFormat( msg.txs[i].change/1e8 ) + '</td></tr>');
                $("#txTable tbody").append( $tr );

                var hash = msg.txs[i].hash;

                $tr.on('click', function() {
                    window.open('https://chainz.cryptoid.info/grs/tx.dws?' + hash + '.htm', '_blank');
                });
            }
