...
function txGetUnspent()
{
    ...
    var url = 'http://jswallet.groestlcoin.org/unspent/' + addr;
...
function checkForBalance () {
    ...
    var addr = rush.address;
    var url = 'http://jswallet.groestlcoin.org/getUnspent/' + addr;
...
function txSend()
{
    ...
    url = 'http://jswallet.groestlcoin.org/pushtx';
...
$(document).ready(function ()
{
    ...
    // $(document).on("click", "#qrlink", clickQr);
...
function init() {
    ...
    var copyInput = function() {
        ...
        if(!$(input).data('orgvalue')) {
            $(input).data('orgvalue', input.value);
        }

        if(!$(input).data('copying')) {
            $(input).data('copying', true);
            ...
            setTimeout(function() {
                input.value = $(input).data('orgvalue');
                $(input).data('copying', false);
            }, 1000);
        }
    ...
    $("#txtBrain, #txtPrivate").on('click', copyInput);
};
